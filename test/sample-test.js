...
describe("DutchAuction",function() {
  ...
  it("Should run a completed auction , ", async function() {
    ...
    const dutchAuction = await DutchAuction.deploy(24, 5000, 1000, 30); //24H Auction
    ...
    var currentPrice = await dutchAuction.priceManager();
    var makeBidTx = await dutchAuction.makeBid(1, {value: currentPrice * 10**9});
    ...
    currentPrice = await dutchAuction.priceManager();

    makeBidTx = await dutchAuction.connect(user1).makeBid(15,{value: currentPrice * 15 * 10**9});
    await makeBidTx.wait()
    ...
    currentPrice = await dutchAuction.priceManager();
    makeBidTx = await dutchAuction.connect(user2).makeBid(20, {value: currentPrice * 20 * 10**9});
    await makeBidTx.wait()
    ...
    await expect(dutchAuction.connect(user2).makeBid(5, {value: currentPrice * 15 * 10**9})).to.be.revertedWith('All tokens Are Sold');
  });
});
